...
public class VirtualChest implements Cloneable {
	...
	public void setItem(int index, org.bukkit.inventory.ItemStack item) {
		setItemStack(
				index,
				(item == null ? null : new net.minecraft.server.ItemStack(item
						.getTypeId(), item.getAmount(), item.getDurability())));
	}
				...
				if (firstPartial == -1) {
					...
					if (firstFree == -1) {
						...
						if (item.getAmount() > getMaxItemStack()) {
							setItem(firstFree,
									new CraftItemStack(item.getTypeId(),
											getMaxItemStack(), item
													.getDurability()));
	...
	public org.bukkit.inventory.ItemStack[] getContents() {
		org.bukkit.inventory.ItemStack[] items = new org.bukkit.inventory.ItemStack[chest
				.getSize()];
		net.minecraft.server.ItemStack[] mcItems = chest.getContents();

		for (int i = 0; i < mcItems.length; i++) {
			items[i] = mcItems[i] == null ? null : new CraftItemStack(
					mcItems[i]);
		}
